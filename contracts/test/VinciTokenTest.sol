...
contract VinciTokenTest is ERC20, Ownable {
    constructor() ERC20("Finci", "FINCI") {
        _mint(address(this), 5 * 10 ** 6 * 10 ** 32);
    }

    event TokenLocked(address indexed beneficiary, uint256 amount, uint256 releaseTime, address contractAddress);

    event SalesContract(uint256 amount, uint256 releaseTime, uint256 vinciTokenPrice, address contractAddress);
     ...
     */
    function lockTokens(address beneficiary, uint256 amount, uint256 releaseTime) public onlyOwner returns (address) {
        ...
        emit TokenLocked(beneficiary, amount, releaseTime, address(token_timelock_contract));
